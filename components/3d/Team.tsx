...
import { Mail, Briefcase, ChevronLeft, ChevronRight, Award, Users, Zap } from "lucide-react";
			...
			<div className="relative z-10 flex flex-col items-center justify-center h-full p-8">
				...
				>
					<div className="w-32 h-32 bg-blue-600 rounded-full flex items-center justify-center shadow-2xl">
						<div className="absolute inset-0 rounded-full bg-blue-500/50 blur-xl animate-pulse" />
					...
					{teamMembers.map((member) => (
						...
						>
							<div className="bg-white rounded-2xl p-6 shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-100">
								{/* Simple colored accent instead of gradient */}
								<div
									className={`absolute top-0 left-0 right-0 h-1 bg-${member.color.split(
										" "
									)[0]
										.replace("from-", "")} rounded-t-2xl`}
								/>

								{/* Content */}
								<div className="text-center">
									<div
										className={`w-16 h-16 mx-auto mb-4 rounded-xl bg-${member.color.split(
											" "
										)[0]
											.replace("from-", "")} flex items-center justify-center group-hover:scale-110 transition-transform`}
									>
				...
				>
					...
					<p className="text-gray-600 text-sm">
						Dedicated professionals driving your success
					</p>
				</motion.div>
				...
				<motion.div
					...
					className="absolute top-10 right-10 w-20 h-20 border-2 border-blue-200 rounded-full opacity-20"
				/>
				<motion.div
					animate={{ rotate: -360 }}
					transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
					className="absolute bottom-10 left-10 w-16 h-16 border-2 border-purple-200 rounded-full opacity-20"
				/>
			</div>
		</div>
	);
